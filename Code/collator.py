...
def pad_adj(x,padlen):
   xlen = x.size(0)
   #x = x.bool().int() 
   if xlen < padlen:
        new_x = x.new_zeros(
            [padlen, padlen], dtype=torch.float32)
        new_x[:xlen, :xlen] = x
        new_x[xlen:, :xlen] = 0
        x = new_x
   return x.unsqueeze(0)
...
class Batch():
    def __init__(self, idx, attn_bias, attn_edge_type, rel_pos, in_degree, out_degree, x, edge_input, y, adj):
...
def collator(items, max_node=512, multi_hop_max_dist=20, rel_pos_max=20):
    ...
    items = [(item.idx, item.attn_bias, item.attn_edge_type, item.rel_pos, item.in_degree, item.adj,
              item.out_degree, item.x, item.edge_input[:, :, :multi_hop_max_dist, :], item.y) for item in items]
    idxs, attn_biases, attn_edge_types, rel_poses, in_degrees, adjs, out_degrees, xs, edge_inputs, ys = zip(
        *items)
    ...
    adj = torch.cat([pad_adj(i,max_node_num + 1) for i in adjs])
